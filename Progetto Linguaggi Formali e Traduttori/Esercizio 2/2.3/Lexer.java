...
public class Lexer {
    ...
    public Token lexical_scan(BufferedReader br) {
        ...
        switch (peek) {
			...
			case '/':
			    readch(br);
				if(peek == '*'){ //se il carattere dopo '/' è '*', entro in un commento
					int i=0;     //contatore che resta 0 fintanto che siamo nel commento
					do{
						readch(br);
						if(peek == '*'){ //se nel commento leggo '*', guardo qual è il prossimo carattere
						    readch(br);
							if(peek == '/') //se è '/', allora aumento i, uscendo dal ciclo
							    i++;}							
						}while(i == 0 && peek != (char)-1);
					if(i == 0){     // se i è ancora a 0, significa che non ho mai chiuso il commento
						System.err.println("Error: comment isn't closed");
						return null;
					} else
					peek = ' ';
				...
				else if(peek == '/'){ //se il carattere dopo '/' è '/' entro in un commento
					do{               //e ci resto finché non vado a capo o leggo EOF
						readch(br);
					}while(peek != (char)-1 && peek != '\n');
					return lexical_scan(br);
				}
				else{                 //se il prossimo carattere non è né '*' né '/', allora è una divisione
					return Token.div;
				}
            ...
            default:
                ...
                if (Character.isLetter(peek)) {
					String parola="";    //leggo lettere, cifre e '_' e le salvo in una parola
					do{
						parola+=peek;
						readch(br);
					...
					do{
						...
						if(i==0 && peek == '0'){ //se la prima cifra è 0...
							readch(br);          //e non ci sono altre cifre dopo di essa 
